...
export let allRecipes = (req: Request, res: Response) => {
    const Recipes = OneRecipe.find((err: any) => {
      if (err) {
        res.send("Error!");
      } else {
        res.send(Recipes);
  ...
  export let getRecipe = (req: Request, res: Response) => {
    const Recipes = OneRecipe.findById(req.params.id, (err: any) => {
      if (err) {
        res.send(err);
      } else {
        res.send(Recipes);
  ...
  export let deleteRecipe = (req: Request, res: Response) => {
    const Recipe = OneRecipe.deleteOne({ _id: req.params.id }, (err: any) => {
      if (err) {
        res.send(err);
      } else {
  ...
  export let updateRecipe = (req: Request, res: Response) => {
    ...
    const book = OneRecipe.findByIdAndUpdate(req.params.id, req.body, (err: any) => {
      if (err) {
        res.send(err);
      } else {
  ...
  export let addRecipe = (req: Request, res: Response) => {
    const Recipe = new OneRecipe(req.body);
    Recipe.save((err: any) => {
      if (err) {
        res.send(err);
      } else {
